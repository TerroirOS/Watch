      ...
      documentTexts.length > 1
        ? [
            ...
            },
            {
              ...
              description:
                "The export declaration (Document 2) does not include a PDO/GI certification number, which is typically required for products claiming protected designation of origin.",
              severity: "high",
            },
          ]
        : [
            {
              ...
              description:
                "Only one document was provided. Upload additional documents (certificates, labels, export records) to enable cross-document discrepancy detection.",
              severity: "low",
            },
          ],
  };
...
export async function processWatchCaseDocuments(
  ...
  const openai = new OpenAI({
    apiKey: watchConfig.openAiApiKey,
  });
